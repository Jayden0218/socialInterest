...
- [X] T045 Dispatch `.github/workflows/android-emulator.yml` and record the result — **G5**. **Run 37, 2026-09-08: 19/19**, every flow on its first attempt, no device drop. It took THREE runs: 35 was 18/19 and found a real defect (the safety sheet could not scroll, so `Block this person` was unreachable — Constitution IV); 36 was **1/19 and was my own regression**, from setting `scroll` on eight screens having measured one. Free on this public repository, and the only place native layout, fonts, safe areas and touch handling are observed
- [X] T046 [P] Write the run record in `docs/verification/runs/` — `2026-09-08-feature-006-record.md`. Names every criterion with the command that measured it, records all THREE device runs rather than only the green one, and lists what is **not** met: 001/SC-011 (video playing), a platform-following light mode, SC-006 (met by construction, never measured as a position delta), iOS, 002/SC-002, real usage, a public deployment
- [X] T047 [P] Update `CLAUDE.md` with what 006 established and what it did not — including the run-36 lesson, and the headline device claim moved from run 29's 17/17 to run 37's 19/19
