...
- [ ] T011 Remove the FR-033 intersection from `apps/api/src/modules/feed/feed.service.ts` and delete `apps/api/src/modules/feed/ranking.ts`, leaving the feed reading a candidate list it is handed. **Single-owner file**
...
- [ ] T015 [US1] Implement `SignalService.record` in `apps/api/src/modules/signals/signal.service.ts` applying every server-side bound from `contracts/signals.md`: clamp dwell to 30s, discard below 3s, one weight per kind per post per session, bounded batch
...
- [ ] T020 [US1] Implement `RankingService.rank` in `apps/api/src/modules/ranking/ranking.service.ts` scoring candidates from the decayed profile. It MUST NOT import the visibility boundary — T004 fails the build if it does
...
- [ ] T022 [US1] Wire `FeedService` in `apps/api/src/modules/feed/feed.service.ts` to call `RankingService` and then `VisibilityFilter.decide` on its output, in that order, per request. **Single-owner file**, and the order is the contract
...
- [ ] T033 [US2] Implement `GET /v1/me/feed-signals` in `apps/api/src/modules/signals/signal.controller.ts`, rendering the explanation from THE SAME WEIGHTS the ranker reads so it cannot drift from the behaviour it describes
...
- [ ] T037 [US2] Extend `apps/api/tests/visibility/matrix.spec.ts` with a row asserting one person's signals are absent from every enumerated surface — **G5**, SC-007. **Single-owner file**
...
- [ ] T044 [US3] Implement `apps/mobile/src/components/Waterfall.tsx` — block-wise, 8 posts per block, each card to the shorter column by accumulated height, blocks virtualised by one `FlatList` (research R6)
...
- [ ] T056 [US3] Run `pnpm --filter @sih/mobile test -- testid-snapshot` and resolve any difference by fixing the SCREEN, never the snapshot — FR-027
- [ ] T057 [P] [US3] Extend `apps/mobile/src/__tests__/touch-target.test.tsx` to all twenty screens — SC-009
...
- [ ] T070 [US5] Verify report and block still reach the server unchanged by running `pnpm --filter @sih/e2e test -- safety`
