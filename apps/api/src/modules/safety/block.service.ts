...
export class BlockService {
  constructor(
    ...
    @Inject(PersonFollowRepository)
    private readonly follows: PersonFollowRepository,
  ) {}
  ...
  async block(blockerId: string, blockedHandle: string): Promise<void> {
        ...
        ? [
            {
              Delete: {
                TableName: table,
                Key: keys.personFollow(blockerId, blocked.userId),
              },
            },
          ]
        : []),
        ...
        ? [
            {
              Delete: {
                TableName: table,
                Key: keys.personFollow(blocked.userId, blockerId),
              },
            },
          ]
        : []),
    ...
    await Promise.all([
      blockerFollows
        ? this.people.incrementCounter(blocked.userId, "followerCount", -1)
        : null,
      blockerFollows
        ? this.people.incrementCounter(blockerId, "followingCount", -1)
        : null,
      blockedFollows
        ? this.people.incrementCounter(blockerId, "followerCount", -1)
        : null,
      blockedFollows
        ? this.people.incrementCounter(blocked.userId, "followingCount", -1)
        : null,
    ]);
  }
