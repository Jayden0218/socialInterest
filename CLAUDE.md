...
| `minio/minio:latest` | **The tag was WITHDRAWN from Docker Hub** (hit 2026-09-11) and took CI down at `docker compose up -d`, with typecheck and lint already green. Docker Hub's message is `pull access denied ... repository does not exist or may require 'docker login'`, which reads like a credentials problem and is not one. Measured against the same registry seconds apart: `latest` not found, `RELEASE.2025-04-22T22-12-26Z` pulls, `amazon/dynamodb-local:latest` unaffected — so neither rate limiting nor an outage. `docker-compose.yml` pins a release tag now. **A floating tag on somebody else's registry means CI's ability to START is outside this repository's control** |
